...
//    dao.hardDeleteSystem(tenantName, dtnSystem1.getId());
  ...
  public void testValidCases() throws Exception
  {
    ...
    validCaseInputs.put(7, new CaseData(20, Arrays.asList("id.like." + sysNameLikeAll, "batch_scheduler.eq." + sys0.getBatchScheduler())));
...
//    validCaseInputs.put(59, new CaseData(1, Arrays.asList("id.like." + sysNameLikeAll, "created.eq." + created1Str)));
    ...
    for (Map.Entry<Integer, CaseData> item : validCaseInputs.entrySet())
    {
      ...
      List<TSystem> searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, DEFAULT_LIMIT,
              orderByListNull,DEFAULT_SKIP, null, null);
  ...
  public void testLimitSkip() throws Exception
  {
    ...
    int limit = -1;
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListNull, DEFAULT_SKIP, startAfterNull, null);
    ...
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListNull, skip, startAfterNull, null);
  ...
  public void testSortingSkip() throws Exception
  {
    ...
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, DEFAULT_LIMIT, orderByListAsc, DEFAULT_SKIP, startAfterNull, null);
    assertEquals(searchResults.size(), numSystems, "Incorrect result count");
    checkOrder(searchResults, 1, numSystems);
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, DEFAULT_LIMIT, orderByListDesc, DEFAULT_SKIP, startAfterNull, null);
    assertEquals(searchResults.size(), numSystems, "Incorrect result count");
    checkOrder(searchResults, numSystems, 1);
    // Sort and check order with limit and no skip
    limit = 4;
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListAsc, DEFAULT_SKIP, startAfterNull, null);
    assertEquals(searchResults.size(), limit, "Incorrect result count");
    checkOrder(searchResults, 1, limit);
    limit = 19;
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListDesc, DEFAULT_SKIP, startAfterNull, null);
    assertEquals(searchResults.size(), limit, "Incorrect result count");
    checkOrder(searchResults, numSystems, numSystems - (limit-1));
    // Sort and check order with limit and skip
    limit = 2;
    skip = 5;
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListAsc, skip, startAfterNull, null);
    assertEquals(searchResults.size(), limit, "Incorrect result count");
    // Should get systems named SrchGet_006 to SrchGet_007
    checkOrder(searchResults, skip + 1, skip + limit);
    limit = 4;
    skip = 3;
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListDesc, skip, startAfterNull, null);
    assertEquals(searchResults.size(), limit, "Incorrect result count");
  ...
  public void testSortingStartAfter() throws Exception
  {
    ...
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListAsc, DEFAULT_SKIP, startAfter, null);
    assertEquals(searchResults.size(), limit, "Incorrect result count");
    // Should get systems named SrchGet_006 to SrchGet_007
    ...
    startAfter = getSysName(testKey, startAfterIdx);
    searchResults = dao.getSystems(tenantName, verifiedSearchList, null, null, limit, orderByListDesc, DEFAULT_SKIP, startAfter, null);
    assertEquals(searchResults.size(), limit, "Incorrect result count");
    // Should get systems named SrchGet_017 to SrchGet_014
