...
public class SystemsServiceImpl implements SystemsService
{
          ...
          throws TapisException, TapisClientException, IllegalStateException, IllegalArgumentException, NotAuthorizedException
  {
    ...
    if (dao.checkForTSystem(systemTenantName, systemName, true))
    {
...
//      skClient.deleteRoleByName(systemTenantName, system.getOwner(), roleNameR);
          ...
          throws TapisException, TapisClientException, IllegalStateException, IllegalArgumentException, NotAuthorizedException, NotFoundException
  {
    ...
    if (!dao.checkForTSystem(systemTenantName, systemName, false))
    {
    ...
    TSystem origTSystem = dao.getTSystem(systemTenantName, systemName);
          ...
          throws TapisException, IllegalStateException, IllegalArgumentException, NotAuthorizedException, NotFoundException, TapisClientException
  {
    ...
    TSystem tmpSystem = dao.getTSystem(systemTenantName, systemName);
   ...
   */
  @Override
  public int softDeleteSystem(AuthenticatedUser authenticatedUser, String systemName) throws TapisException, NotAuthorizedException, TapisClientException
  {
    ...
    if (!dao.checkForTSystem(systemTenantName, systemName, false)) return 0;

    // ------------------------- Check service level authorization -------------------------
    checkAuth(authenticatedUser, op, systemName, null, null, null);

    TSystem system = dao.getTSystem(systemTenantName, systemName);
   ...
   * @throws NotAuthorizedException - unauthorized
   */
  public int hardDeleteSystem(AuthenticatedUser authenticatedUser, String systemName)
          throws TapisException, TapisClientException, NotAuthorizedException
  {
    ...
    if (!dao.checkForTSystem(systemTenantName, systemName, true)) return 0;
...
//                              null, SERVICE_NAME_SYSTEMS, svcMasterTenant, null, siteId, svcJwt);
   ...
   */
  @Override
  public boolean checkForSystem(AuthenticatedUser authenticatedUser, String systemName) throws TapisException, NotAuthorizedException, TapisClientException
  {
...
//    BooleanExpression searchAST;
   ...
   */
  @Override
  public SystemBasic getSystemBasic(AuthenticatedUser authenticatedUser, String systemName)
          throws TapisException, NotAuthorizedException, TapisClientException
  {
    SystemOperation op = SystemOperation.read;
    if (authenticatedUser == null) throw new IllegalArgumentException(LibUtils.getMsg("SYSLIB_NULL_INPUT_AUTHUSR"));
    ...
    // Retrieve the system
    SystemBasic result = dao.getSystemBasic(systemTenantName, systemName);
    return result;
  }
          ...
          throws TapisException, TapisClientException
  {
    ...
    // Get all allowed systems matching the search conditions
    List<SystemBasic> systems = dao.getSystemsBasic(authenticatedUser.getTenantId(), verifiedSearchList, null, allowedSystemIDs,
                                                    limit, sortBy, sortDirection, skip, startAfter);
    return systems;
  }
          ...
          throws TapisException, TapisClientException
  {
    ...
    // Get all allowed systems matching the search conditions
    List<SystemBasic> systems = dao.getSystemsBasic(authenticatedUser.getTenantId(), null, searchAST, allowedSystemIDs,
                                                    limit, sortBy, sortDirection, skip, startAfter);
    return systems;
  }
          ...
          throws TapisException, NotAuthorizedException, TapisClientException
  {
    ...
    int changeCount = 0;
    // If system does not exist or has been soft deleted then return 0 changes
    if (!dao.checkForTSystem(systemTenantName, systemName, false)) return changeCount;
...
//    if (!dao.checkForTSystem(systemTenantName, systemName))
//      throw new TapisException(LibUtils.getMsgAuth("SYSLIB_NOT_FOUND", authenticatedUser, systemName));
      ...
      throws TapisException, TapisClientException, NotAuthorizedException, IllegalStateException
  {
    ...
    {
      // User check
      checkAuthUser(authenticatedUser, operation, null, null, systemName, owner, targetUser, perms);
      return;
    }
  ...
  private void checkAuthUser(AuthenticatedUser authenticatedUser, SystemOperation operation,
                             ...
                             String systemName, String owner, String targetUser, Set<Permission> perms)
          throws TapisException, TapisClientException, NotAuthorizedException, IllegalStateException
  {
    ...
    if (StringUtils.isBlank(owner)) owner = dao.getTSystemOwner(tenantName, systemName);
    ...
    switch(operation) {
      ...
      case getPerms:
        if (owner.equals(userName) || hasAdminRole(authenticatedUser, tenantName, userName) ||
              isPermittedAny(authenticatedUser, tenantName, userName, systemName, READMODIFY_PERMS)) return;
        break;
      case modify:
        if (owner.equals(userName) || hasAdminRole(authenticatedUser, tenantName, userName) ||
                isPermitted(authenticatedUser, tenantName, userName, systemName, Permission.MODIFY)) return;
        break;
      case execute:
        if (owner.equals(userName) || hasAdminRole(authenticatedUser, tenantName, userName) ||
                isPermitted(authenticatedUser, tenantName, userName, systemName, Permission.EXECUTE)) return;
        break;
      case revokePerms:
        if (owner.equals(userName) || hasAdminRole(authenticatedUser, tenantName, userName) ||
                (userName.equals(targetUser) &&
                        allowUserRevokePerm(authenticatedUser, tenantName, userName, systemName, perms))) return;
        break;
      case setCred:
      case removeCred:
        if (owner.equals(userName) || hasAdminRole(authenticatedUser, tenantName, userName) ||
...
//    return skClient.hasRole(tenantName, userName, SYSTEMS_ADMIN_ROLE);
  ...
  private boolean isPermitted(AuthenticatedUser authenticatedUser, String tenantToCheck, String userToCheck,
                              String systemName, Permission perm)
          throws TapisException, TapisClientException
  {
    // Use tenant and user from authenticatedUsr or optional provided values
    String tenantName = (StringUtils.isBlank(tenantToCheck) ? authenticatedUser.getTenantId() : tenantToCheck);
    String userName = (StringUtils.isBlank(userToCheck) ? authenticatedUser.getName() : userToCheck);
    var skClient = getSKClient(authenticatedUser);
    String permSpecStr = getPermSpecStr(tenantName, systemName, perm);
  ...
  private boolean isPermittedAny(AuthenticatedUser authenticatedUser, String tenantToCheck, String userToCheck,
                                 String systemName, Set<Permission> perms)
          throws TapisException, TapisClientException
  {
    // Use tenant and user from authenticatedUsr or optional provided values
    String tenantName = (StringUtils.isBlank(tenantToCheck) ? authenticatedUser.getTenantId() : tenantToCheck);
    ...
    for (Permission perm : perms) {
      permSpecs.add(getPermSpecStr(tenantName, systemName, perm));
    }
          ...
          throws TapisException, TapisClientException
  {
    ...
    String tenantName = (StringUtils.isBlank(tenantToCheck) ? authenticatedUser.getTenantId() : tenantToCheck);
    String userName = (StringUtils.isBlank(userToCheck) ? authenticatedUser.getName() : userToCheck);
    if (perms.contains(Permission.MODIFY)) return isPermitted(authenticatedUser, tenantName, userName, systemName, Permission.MODIFY);
    if (perms.contains(Permission.READ)) return isPermittedAny(authenticatedUser, tenantName, userName, systemName, READMODIFY_PERMS);
